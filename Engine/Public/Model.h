...
public:
	...
	_uint Get_Current_KeyFrame_Index(const char* pNameKey);
	HRESULT Update_Animation(_float fTimeDelta, OUT _float4x4* pMatW = nullptr, const char* pRootNodeName = nullptr, class CNavigation* pNaviCom = nullptr, IN OBJECT_DIR  eDir = OBJECT_DIR::DIR_F, int iChannelIdx = -1, _float OffsetMul = 1.f);
