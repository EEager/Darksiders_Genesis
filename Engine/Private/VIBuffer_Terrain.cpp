...
HRESULT CVIBuffer_Terrain::NativeConstruct_Prototype(const _tchar* pShaderFilePath, _uint iNumVerticesX, _uint iNumVerticexZ)
{
	...
	for (_uint i = 0; i < m_iNumVerticesZ; ++i)
	{
		for (_uint j = 0; j < m_iNumVerticesX; ++j)
		{
			...
			((VTXNORTEX*)m_pVertices)[iIndex].vPosition = _float3((float)j, -67.f, (float)i);
...
CVIBuffer_Terrain* CVIBuffer_Terrain::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, const _tchar* pShaderFilePath, _uint iNumVerticesX, _uint iNumVerticexZ)
{
	CVIBuffer_Terrain* pInstance = new CVIBuffer_Terrain(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(pShaderFilePath, iNumVerticesX, iNumVerticexZ)))
	{
		MSG_BOX("Failed To Creating CVIBuffer_Terrain");
		Safe_Release(pInstance);
	}
