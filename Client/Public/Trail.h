...
protected:
	...
	int m_TrailTextureIdx = 0.f; // 트레일 텍스쳐
	int m_NoiseTextureIdx = 0.f; // 노이즈 텍스쳐
...
public:
	...
	virtual HRESULT NativeConstruct(void* pArg);
	virtual _int Tick(_float fTimeDelta);
	_int CTrail_War_Sword::MyTick(_float fTimeDelta, _fmatrix* pBonemMat = nullptr); 
	virtual _int LateTick(_float fTimeDelta);
	virtual HRESULT Render(_uint iPassIndex = 0);
	virtual HRESULT PostRender(unique_ptr<SpriteBatch>& m_spriteBatch, unique_ptr<SpriteFont>& m_spriteFont);
...
// CTrail_War_Dash
//-------------------------------------------------
class CTrail_War_Dash final : public CTrail
{
private:
	explicit CTrail_War_Dash(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CTrail_War_Dash(const CTrail_War_Dash& rhs);
	virtual ~CTrail_War_Dash() = default;
public:
	virtual HRESULT NativeConstruct_Prototype();
	virtual HRESULT NativeConstruct(void* pArg);
	virtual _int Tick(_float fTimeDelta);
	_int MyTick(_float fTimeDelta, _fmatrix* pBonemMat = nullptr); // Trail을 소유하고 있는 객체에서 pBonemMat를 채워주도록하자
	virtual _int LateTick(_float fTimeDelta);
	virtual HRESULT Render(_uint iPassIndex = 0);
	virtual HRESULT PostRender(unique_ptr<SpriteBatch>& m_spriteBatch, unique_ptr<SpriteFont>& m_spriteFont);
...
public:
	static CTrail_War_Dash* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	virtual CGameObject* Clone(void* pArg) override;
	virtual void Free() override;
};
END